...
func (gq *GuildQuery) QueryChannels() *ChannelQuery {
...
func (gq *GuildQuery) WithChannels(opts ...func(*ChannelQuery)) *GuildQuery {
	query := (&ChannelClient{config: gq.config}).Query()
...
func (gq *GuildQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Guild, error) {
	...
	if query := gq.withChannels; query != nil {
		if err := gq.loadChannels(ctx, query, nodes,
			func(n *Guild) { n.Edges.Channels = []*Channel{} },
			func(n *Guild, e *Channel) { n.Edges.Channels = append(n.Edges.Channels, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GuildQuery) loadChannels(ctx context.Context, query *ChannelQuery, nodes []*Guild, init func(*Guild), assign func(*Guild, *Channel)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Guild)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(channel.FieldGuildID)
	}
	query.Where(predicate.Channel(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(guild.ChannelsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "guild_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
