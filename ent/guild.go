...
func (e GuildEdges) ChannelsOrErr() ([]*Channel, error) {
	if e.loadedTypes[0] {
...
func (*Guild) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case guild.FieldID, guild.FieldName:
...
// QueryChannels queries the "channels" edge of the Guild entity.
func (gu *Guild) QueryChannels() *ChannelQuery {
	return NewGuildClient(gu.config).QueryChannels(gu)
}
...
func (gu *Guild) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", gu.Active))
	builder.WriteString(", ")
