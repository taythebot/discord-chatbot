...
func (mq *MessageQuery) QueryOwner() *ChannelQuery {
	query := (&ChannelClient{config: mq.config}).Query()
...
func (mq *MessageQuery) WithOwner(opts ...func(*ChannelQuery)) *MessageQuery {
	query := (&ChannelClient{config: mq.config}).Query()
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := mq.withOwner; query != nil {
		if err := mq.loadOwner(ctx, query, nodes, nil,
			func(n *Message, e *Channel) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MessageQuery) loadOwner(ctx context.Context, query *ChannelQuery, nodes []*Message, init func(*Message), assign func(*Message, *Channel)) error {
